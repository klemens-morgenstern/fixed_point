...
constexpr fp<wl, fl, sign, r_mode> operator+(fp<wl, fl, sign, r_mode> fp) {return fp;}

template<long wl, long fl, rounding_mode r_mode>
constexpr fp<wl, fl, signed, r_mode> operator-(fp<wl, fl, signed, r_mode> fp)
{
	return -fp.value();
}

template<long wl, long fl, rounding_mode r_mode>
constexpr fp<wl+1, fl, signed, r_mode> operator-(fp<wl, fl, unsigned, r_mode> fp)
{
	typedef typename fp<wl+1, fl, signed, r_mode>::int_type int_type;
