...
template<	long wl, long fl, typename _sign_t, rounding_mode r_mode, narrow fp_convs,
	...
	struct fp_t	: fp_int_conv<wl,fl, _sign_t, r_mode, fp_convs, sign_change, flt_convs, int_convs>
	{
		...
		constexpr fp_t(const fp_t<wl2, fl2, _sign_t2, r_mode2, fp_convs2, sign_change2, flt_convs2,  int_convs2> &fp) : fp_t(fp.get_fp(fp_t())) {};

		fp_t() = default;
		fp_t(const fp_t &) = default;
		fp_t& operator= (const fp_t& ) = default;
	};
}
